...
- **Hidden "Connected" status cards**: Provider status cards now only show on error — the green "Connected" line was unnecessary noise
...
- **Provider-specific peak visuals**: Peak-hours styling is now applied only to Claude metrics. Copilot and Codex progress bars/menu-bar meters no longer get amber stripe overlays or peak background tinting during Anthropic peak windows.
- **Popover consistency for non-Claude providers**: Usage rows now gate peak stripe overlays by provider, so Copilot and Codex keep a stable appearance at all times.
...
- **Compact Reset + Elapsed Line**: Combined the reset time and elapsed percentage into a single line in usage rows (`Zurücksetzen Today 15:59  |  29% verstrichen`), reducing overall popover height
